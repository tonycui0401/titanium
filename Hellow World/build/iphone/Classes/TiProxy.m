...
//Rare exceptions to indicate a bug in the _hellowworld code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_hellowworld = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_hellowworld]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.hellowworld.module"
